...
public class ModelDiopsittacaNobilis extends ModelBase {
    ...
    private ModelDiopsittacaNobilis.State state = ModelDiopsittacaNobilis.State.STANDING;

    public ModelDiopsittacaNobilis() {
    		...
    		float headPitch, float scaleFactor, Entity entityIn) 
    {
        ...
        if (this.state != ModelDiopsittacaNobilis.State.FLYING)
        {
            if (this.state == ModelDiopsittacaNobilis.State.SITTING)
            {
                return;
            }

            if (this.state == ModelDiopsittacaNobilis.State.PARTY)
            {
    ...
    public void setLivingAnimations(EntityLivingBase entitylivingbaseIn, float p_78086_2_, float p_78086_3_, float partialTickTime)
    {
        ...
        if (entitylivingbaseIn instanceof EntityParrot)
        {
            ...
            if (entityparrot.isPartying()) 
            {
                ...
                this.state = ModelDiopsittacaNobilis.State.PARTY;
                return;
            }
            ...
            if (entityparrot.isSitting())
            {
                ...
                this.state = ModelDiopsittacaNobilis.State.SITTING;
            }
            else if (!entityparrot.onGround)
            {
                this.leftleg.rotateAngleX += ((float)Math.PI * 2F / 9F);
                this.rightleg.rotateAngleX += ((float)Math.PI * 2F / 9F);
                this.state = ModelDiopsittacaNobilis.State.FLYING;
                //System.out.println("Flying");
            }
            else
            {
                this.state = ModelDiopsittacaNobilis.State.STANDING;
